...
import styles from "./TouristVisaFormScreen.module.css";
...
// Matches the "1.5MB max per file" label on the passport/photo uploaders —
...
// MobileFormScreen.
export default function TouristVisaFormScreen({
        ...
        {step === 1 && (
          <form onSubmit={goNext} noValidate>
            <div className={styles.field}>
              ...
              <input className={err(attempted1 && !applicantValid)} type="text" placeholder="As on passport" value={applicant} onChange={(e) => setApplicant(e.target.value)} />
              {attempted1 && !applicantValid && <span className={styles.fieldError}>Please enter the applicant&apos;s full name.</span>}
            </div>

            <div className={styles.field}>
              <label>Enter Valid Email <span className={styles.req}>*</span></label>
              <input className={err(attempted1 && !emailValid)} type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} />
              {attempted1 && !emailValid && <span className={styles.fieldError}>Please enter a valid email address.</span>}
            </div>

            <div className={styles.field}>
              <label>Enter Mobile No. <span className={styles.req}>*</span></label>
              <div className={styles.phoneRow}>
                <CountryCodeSelect value={mobileCountry} onChange={setMobileCountry} label="Mobile country code" />
                <input className={err(attempted1 && !mobileValid)} type="tel" inputMode="tel" placeholder="50 000 0000" value={mobile} onChange={(e) => setMobile(e.target.value)} />
              </div>
              {attempted1 && !mobileValid && <span className={styles.fieldError}>Please enter a valid mobile number.</span>}
            </div>

            <div className={styles.field}>
              <label>WhatsApp No. <span className={styles.req}>*</span></label>
              <div className={styles.phoneRow}>
                <CountryCodeSelect value={whatsappCountry} onChange={setWhatsappCountry} label="WhatsApp country code" />
                <input className={err(attempted1 && !whatsappValid)} type="tel" inputMode="tel" placeholder="50 000 0000" value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} />
              </div>
              {attempted1 && !whatsappValid && <span className={styles.fieldError}>Please enter a valid WhatsApp number.</span>}
            </div>

            <div className={styles.field}>
              <label>Select Your Nationality <span className={styles.req}>*</span></label>
              <select className={`${styles.select} ${attempted1 && !nationalityValid ? styles.inputError : ""}`} value={nationality} onChange={(e) => setNationality(e.target.value)}>
            ...
            <div className={styles.field}>
              ...
              <input className={styles.input} type="date" value={travelDate} onChange={(e) => setTravelDate(e.target.value)} />
            </div>
          ...
          <form onSubmit={submit} noValidate>
            <div className={styles.passengerGroup}>
              ...
              {adults.map((a, i) => (
                <div key={i} className={styles.passengerCard}>
                  <div className={styles.field}>
                    <label>Full Name — Adult {i + 1} <span className={styles.req}>*</span></label>
                    <input className={err(attempted2 && a.name.trim().length <= 1)} type="text" placeholder="As on passport" value={a.name} onChange={(e) => updateAdult(i, "name", e.target.value)} />
                    {attempted2 && a.name.trim().length <= 1 && <span className={styles.fieldError}>Please enter this passenger&apos;s full name.</span>}
                  </div>
                  <div className={styles.field} style={{ marginBottom: 0 }}>
                    <label>Date of Birth — Adult {i + 1} <span className={styles.req}>*</span></label>
                    <input className={err(attempted2 && !a.dob)} type="date" value={a.dob} onChange={(e) => updateAdult(i, "dob", e.target.value)} />
            ...
            <div className={styles.passengerGroup}>
              ...
              {children.map((c, i) => (
                <div key={i} className={styles.passengerCard}>
                  <div className={styles.field}>
                    <label>Full Name — Child {i + 1} <span className={styles.req}>*</span></label>
                    <input className={err(attempted2 && c.name.trim().length <= 1)} type="text" placeholder="As on passport" value={c.name} onChange={(e) => updateChild(i, "name", e.target.value)} />
                    {attempted2 && c.name.trim().length <= 1 && <span className={styles.fieldError}>Please enter this passenger&apos;s full name.</span>}
                  </div>
                  <div className={styles.field} style={{ marginBottom: 0 }}>
                    <label>Date of Birth — Child {i + 1} <span className={styles.req}>*</span></label>
                    <input className={err(attempted2 && !c.dob)} type="date" value={c.dob} onChange={(e) => updateChild(i, "dob", e.target.value)} />
            ...
            <div className={styles.field}>
              <label>Passport Copies of All Passengers <span className={styles.req}>*</span></label>
              <input
              ...
              >
                ...
                <span className={styles.upTxt}>
                  ...
                  <span>JPG/JPEG only, 1.5MB max per file</span>
                </span>
            ...
            <div className={styles.field}>
              <label>Photos with White Background of All Passengers <span className={styles.req}>*</span></label>
              <input
                ref={photoInputRef}
                type="file"
                multiple
